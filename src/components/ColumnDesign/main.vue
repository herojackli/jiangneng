              ...
              <template v-if="columnData.type==2">
                ...
                <template v-if="columnData.treeDataSource==='api'">
                  <el-form-item label="数据接口">
                    <HONGAN4D-TreeSelect :options="dataInterfaceSelector"
...
export default {
  ...
  created() {
    ...
    loop(getDrawingList())
    this.list = list
    let columnOptions = list.filter(o => noColumnShowList.indexOf(o.__config__.hongan4dKey) < 0)
  ...
  methods: {
    ...
    */
    ...
    addCustomBtn() {
      const id = this.hongan4d.idGenerator()
