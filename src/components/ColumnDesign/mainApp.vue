  ...
  <div class="columnDesign-container">
    <div class="main-board">
      <div class="search-box">
        ...
        <el-table :data="columnData.searchList" class="HONGAN4D-common-table" ref="dragTableSearch"
      ...
      <div class="sort-box">
        ...
        <el-table :data="columnData.sortList" class="HONGAN4D-common-table" ref="dragTableSort"
          :row-key="row=>row.prop">
          <el-table-column align="center" label="拖动" width="50">
            <template>
              <i class="drag-handler icon-ym icon-ym-darg" style="cursor: move;font-size:20px"
      ...
      <div class="column-box">
        ...
        <el-table :data="columnData.columnList" class="HONGAN4D-common-table" ref="dragTable"
          :row-key="row=>row.prop">
          <el-table-column align="center" label="拖动" width="50">
            <template>
              <i class="drag-handler icon-ym icon-ym-darg" style="cursor: move;font-size:20px"
    ...
    <div class="right-board">
      ...
      <div class="field-box">
        <div class="columnList" v-show="currentTab==='sort'">
          <el-table :data="sortOptions" class="HONGAN4D-common-table" height="100%"
        ...
        <div class="columnList" v-show="currentTab==='search'">
          <el-table :data="searchOptions" class="HONGAN4D-common-table" height="100%"
        ...
        <div class="columnList" v-show="currentTab==='field'">
          <el-table :data="columnOptions" class="HONGAN4D-common-table" height="100%"
...
const getSearchType = item => {
  const hongan4dKey = item.__config__.hongan4dKey
  // 等于-1  模糊-2  范围-3
  const fuzzyList = [...useInputList]
  const RangeList = [...useDateList, 'time', 'date', 'numInput', 'calculate']
  if (RangeList.includes(hongan4dKey)) return 3
  if (fuzzyList.includes(hongan4dKey)) return 2
  return 1
}
  ...
  created() {
    ...
    const loop = (data, parent) => {
      ...
      if (data.__config__ && data.__config__.hongan4dKey) {
        ...
        if (data.__config__.layout === "colFormItem" && data.__vModel__ && data.__vModel__.indexOf('_hongan4d_') < 0 && visibility) {
          list1.push(data)
        }
      }
    }
    loop(getDrawingList())
    this.list = list
    let options = list.filter(o => noColumnShowList.indexOf(o.__config__.hongan4dKey) < 0)
    let searchOptions = list.filter(o => noSearchList.indexOf(o.__config__.hongan4dKey) < 0)
    let sortOptions = list1.filter(o => noColumnShowList.indexOf(o.__config__.hongan4dKey) < 0)
    this.columnOptions = options.map(o => ({
      label: o.__config__.label,
      prop: o.__vModel__,
      align: 'left',
      hongan4dKey: o.__config__.hongan4dKey,
      sortable: false,
      width: null
    }));
