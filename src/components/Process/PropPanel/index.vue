...
<template>
  <el-drawer size="550px" class="drawer HONGAN4D-common-drawer" :visible.sync="visible" @close="cancel"
      ...
      <template v-for="(item, index) in pconditions">
        <el-row class="condition-list" :key="index">
          ...
          <el-col :span="7" class="filedValue">
            ...
            <template v-else-if="['date','createTime', 'modifyTime'].includes(item.hongan4dKey)">
            ...
            <template v-else-if="['comSelect','currOrganize'].includes(item.hongan4dKey)">
              <comSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionListChange(arguments,item)" />
            </template>
            <template v-else-if="['depSelect'].includes(item.hongan4dKey)">
              <depSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="['userSelect','createUser','modifyUser'].includes(item.hongan4dKey)">
              <userSelect v-model="item.filedValue" placeholder="请选择" hasSys clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="['posSelect','currPosition'].includes(item.hongan4dKey)">
              <posSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="item.hongan4dKey==='address'">
              <HONGAN4DAddress v-model="item.filedValue" placeholder="请选择" :level="item.level" clearable
                @change="onConditionListChange(arguments,item)" />
            </template>
            <template v-else>
          ...
          <el-scrollbar class="config-scrollbar">
            <el-form label-position="top" :model="subFlowForm" class="pd-10">
              ...
              <el-form-item label="子流程表单">
                <HONGAN4D-TreeSelect :options="flowOptions" v-model="subFlowForm.flowId"
      ...
      <el-tabs style="height:100%;">
            ...
            <el-form label-position="top" class="pd-10">
              <el-form-item label="操作设置">
                ...
                <div class="per-cell" v-if="startForm.hasPrintBtn">
                  ...
                  <HONGAN4D-TreeSelect :options="printTplList" v-model="startForm.printId"
        ...
        <el-tab-pane label="表单权限">
          <div class="form-auth-table">
            <el-table :data="getFormOperates()" class="HONGAN4D-common-table" size="mini" height="100%">
            ...
            <el-form label-position="top" class="pd-10">
              <el-form-item label="操作设置">
                ...
                <div class="per-cell" v-if="approverForm.hasPrintBtn">
                  <p style="width:112px"></p>
                  <HONGAN4D-TreeSelect :options="printTplList" v-model="approverForm.printId"
                    placeholder="请选择打印模板" lastLevel clearable></HONGAN4D-TreeSelect>
                </div>
              </el-form-item>
    ...
    <el-dialog title="数据传递" :close-on-click-modal="false" :visible.sync="ruleVisible"
      class="HONGAN4D-dialog HONGAN4D-dialog_center rule-dialog" lock-scroll append-to-body width='700px'>
...
const requiredDisabled = (hongan4dKey) => {
  return ['billRule', 'createUser', 'createTime', 'modifyTime', 'modifyUser', 'currPosition', 'currOrganize', 'table'].includes(hongan4dKey)
}
const getDataType = (data) => {
  if (!data.__config__ || !data.__config__.hongan4dKey) return ''
  const hongan4dKey = data.__config__.hongan4dKey
  if (['numInput', 'date', 'rate', 'slider'].includes(hongan4dKey)) {
    return 'number'
  } else if (['checkbox', 'uploadFz', 'uploadImg', 'cascader', 'comSelect', 'address'].includes(hongan4dKey)) {
    return 'array'
  } else if (['select', 'depSelect', 'posSelect', 'userSelect', 'treeSelect'].includes(hongan4dKey)) {
...
},
{
  ...
  __vModel__: 'hongan4dFlowFullName',
},
{
  __config__: {
    label: '任务标题',
  ...
  __vModel__: 'hongan4dTaskFullName',
},
{
  __config__: {
    label: '发起用户ID',
  ...
  __vModel__: 'hongan4dLaunchUserId',
},
{
  __config__: {
    label: '发起用户名',
  ...
  __vModel__: 'hongan4dLaunchUserName',
},
{
  __config__: {
    label: '当前操作用户ID',
  ...
  __vModel__: 'hongan4dFlowOperatorUserId',
},
{
  __config__: {
    label: '当前操作用户名',
  ...
  __vModel__: 'hongan4dFlowOperatorUserName',
}]
export default {
  ...
  computed: {
    ...
    usedFormItems() {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__vModel__ && data.__config__.hongan4dKey !== 'table') list.push(data)
      }
      loop(getDrawingList())
  ...
  methods: {
    ...
    initFormOperates(target) {
      ...
      if (!formOperates.length) {
        const loop = (data, parent) => {
          if (!data) return
          if (data.__vModel__) {
            const isTableChild = parent && parent.__config__ && parent.__config__.hongan4dKey === 'table'
            res.push({
              ...
              requiredDisabled: requiredDisabled(data.__config__.hongan4dKey) || data.__config__.required,
              hongan4dKey: data.__config__.hongan4dKey,
    ...
    */
    ...
    fieldNameChange(val, item, i) {
      ...
      item.hongan4dKey = obj.__config__.hongan4dKey
      item = { ...item, ...obj }
    ...
    onConditionDateChange(val, item) {
      ...
      item.filedLabel = this.hongan4d.toDate(val, format)
    },
