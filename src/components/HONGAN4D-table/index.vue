<template>
  <div class="tableContainer">
    <el-table v-if="!refreshTable" :data="data" ref="HONGAN4DTable" class="HONGAN4D-common-table"
      ...
      <el-table-column prop="index" type="index" width="50" label="序号" v-if="hasNO"
        align="center" />
      <hongan4d-table-column :columns="columns" :columnList="columnList" v-if="customColumn" />
      <template v-else>
        <slot />
      </template>
      <el-table-column align="center" fixed="right" width="36" v-if="customColumn">
        <template slot="header">
          <el-link icon="icon-ym icon-ym-options HONGAN4D-common-head-icon" :underline="false"
            @click="showDrawer" />
        </template>
      </el-table-column>
      <template slot="empty">
...
<script>
import Hongan4dTableColumn from './Column'
import ColumnSettings from './ColumnSettings'
export default {
  name: 'HONGAN4D-table',
  components: { Hongan4dTableColumn, ColumnSettings },
  props: {
    data: {
      type: Array,
      default: () => []
  ...
  methods: {
    ...
    getColumns() {
      ...
      const cacheList = this.hongan4d.storageGet(this.menuId)
      if (!cacheList) {
    ...
    doLayout() {
      setTimeout(() => {
        this.$nextTick(() => {
          this.$refs && this.$refs.HONGAN4DTable && this.$refs.HONGAN4DTable.doLayout()
        })
      }, 50)
    },
    setColumn(list) {
      // this.loading = true
      this.hongan4d.storageSet({ [this.menuId]: list })
