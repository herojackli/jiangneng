...
const tags = {
  ...
  'HONGAN4D-UploadImg': function(el) {
  ...
  'HONGAN4D-UploadFz': function(el) {
    const tag = this['HONGAN4D-UploadImg'](el)
    const buttonText = `buttonText="${el.buttonText}"`
    return addPropToTag(tag, buttonText)
  },
  'HONGAN4D-Quill': function(el) {
    const { tag, vModel, placeholder } = attrBuilder(el)
