<template>
  <div class="hongan4d-table-box" :class="[config.__config__.type]">
    <div class="HONGAN4D-common-title" v-if="config.__config__.showTitle && config.__config__.label">
      <h2>{{ config.__config__.label }}</h2>
    </div>
    <el-table :data="tableFormData" class="HONGAN4D-common-table" @cell-click="focusInput"
      ...
      <template v-for="(head, cindex) in tableData">
        <el-table-column :key="head.__config__.formId" :min-width="head['min-width']"
          ...
          <template slot-scope="scope">
            ...
            <template v-if="['select', 'checkbox','radio'].includes(head.__config__.hongan4dKey)">
            ...
            <!-- 单行输入 -->
            <template v-else-if="head.__config__.hongan4dKey==='comInput'">
  ...
  methods: {
        ...
        if (dyOptionsList.indexOf(config.hongan4dKey) > -1) {
          ...
          if (config.dataType === 'dynamic') {
            ...
            getDataInterfaceRes(config.propsUrl).then(res => {
              let realData = this.hongan4d.interfaceDataHandler(res.data)
    ...
    setTableFormData(prop, value) {
      ...
      for (let i = 0; i < activeRow.length; i++) {
        ...
        if (activeRow[i].__vModel__.indexOf('_hongan4dRelation_') >= 0) {
          vModel = activeRow[i].__vModel__.substring(0, activeRow[i].__vModel__.indexOf('_hongan4dRelation_'))
        }
        if (vModel === prop) {
    ...
    getTableFieldOptions(prop) {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        if (this.tableData[i].__vModel__ === prop) {
          ...
          let isTreeSelect = item.__config__.hongan4dKey === 'treeSelect' || item.__config__.hongan4dKey === 'cascader'
    ...
    onFormBlur(rowIndex, colIndex, tag) {
      ...
      if (data && data.on && data.on.blur) {
        const func = this.hongan4d.getScriptFunc.call(this, data.on.blur)
        if (!func) return
        func.call(this, {
          data: null,
          ...this.parameter
    ...
    onFormDataChange(rowIndex, colIndex, tag, params) {
      ...
      if (data && data.on && data.on.change) {
        const func = this.hongan4d.getScriptFunc.call(this, data.on.change)
        if (!func) return
        let value = ''
        if (['select', 'radio', 'checkbox'].includes(data.hongan4dKey)) {
          const options = data.options
          if (data.config.multiple || data.hongan4dKey === 'checkbox') {
            let _value = []
        ...
        } else if (data.hongan4dKey === 'numInput') {
          value = params[0]
        } else {
    ...
    getConfById(formId, rowIndex) {
      ...
      if (['relationForm', 'popupSelect'].includes(itemConfig.hongan4dKey)) {
        newObj['field'] = item.__vModel__ + '_hongan4dRelation_' + rowIndex
      }
      if (['relationFormAttr', 'popupAttr'].includes(itemConfig.hongan4dKey)) {
        let prop = newObj['relationField'].split('_hongan4dTable_')[0]
        newObj['relationField'] = prop + '_hongan4dRelation_' + rowIndex
      }
      return newObj
    },
    /**
    ...
    getEmptyRow(val, rowIndex) {
      return this.tableData.map((t, index) => {
        let res = {
          ...
          regErrorText: '',
          on: t.on || {},
          hongan4dKey: t.__config__.hongan4dKey,
          __vModel__: ['relationForm', 'popupSelect'].includes(t.__config__.hongan4dKey) ? t.__vModel__ + '_hongan4dRelation_' + rowIndex : t.__vModel__,
    ...
    getTableValue() {
      return this.tableFormData.map(row => row.reduce((p, c) => {
        ...
        if (c.__vModel__ && c.__vModel__.indexOf('_hongan4dRelation_') >= 0) {
          str = c.__vModel__.substring(0, c.__vModel__.indexOf('_hongan4dRelation_'))
        }
        p[str] = c.value
