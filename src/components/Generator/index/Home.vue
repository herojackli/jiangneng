  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (this.$store.getters.hasTable && config.layout === "colFormItem" && !noVModelList.includes(config.hongan4dKey) && !e.__vModel__) {
              reject({ msg: `请选择${config.label}的控件字段`, target: 1 })
              break
            }
            if (config.hongan4dKey === 'billRule') {
              if (!config.rule) {
                reject({ msg: '单据组件“选择模板”属性为必填项', target: 1 })
                break
              }
            }
            if (config.hongan4dKey === 'relationForm') {
              if (!e.modelId) {
            ...
            if (config.hongan4dKey === 'popupSelect') {
              if (!e.interfaceId) {
    ...
    shouldClone(to, from, target, event, conf) {
      ...
      if (conf.__config__.rowType === 'table') {
        if (noTableAllowList.includes(targetConf.__config__.hongan4dKey)) {
          // if (this.showTip) {
    ...
    createIdAndKey(item) {
      ...
      if (config.layout === 'colFormItem') {
        if (!this.$store.getters.hasTable) {
          // 分割线和按钮不加vModel
          if (noVModelList.indexOf(config.hongan4dKey) < 0) {
            item.__vModel__ = `${config.hongan4dKey}Field${this.idGlobal}`
          }
        } else {
          if (noVModelList.indexOf(config.hongan4dKey) < 0) {
            item.__vModel__ = ""
