        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px"
          ...
          <template v-if="activeData.__config__">
            <template v-if="$store.getters.hasTable">
              <template v-if="activeData.__config__.hongan4dKey==='table'">
                <el-form-item
                  v-if="activeData.__vModel__!==undefined && !noVModelList.includes(activeData.__config__.hongan4dKey)"
                  label="控件字段">
            ...
            <HONGAN4DComInput v-if="activeData.__config__.hongan4dKey==='comInput'"
              :active-data="activeData" />
            <HONGAN4D-textarea v-if="activeData.__config__.hongan4dKey==='textarea'"
              :active-data="activeData" />
            <HONGAN4DText v-if="activeData.__config__.hongan4dKey==='HONGAN4DText'" :active-data="activeData" />
            <HONGAN4DNumInput v-if="activeData.__config__.hongan4dKey==='numInput'"
              :active-data="activeData" />
            <HONGAN4DAmount v-if="activeData.__config__.hongan4dKey==='HONGAN4DAmount'"
              :active-data="activeData" />
            <HONGAN4DPsdInput v-if="activeData.__config__.hongan4dKey==='PsdInput'"
              :active-data="activeData" />
            <HONGAN4DRadio v-if="activeData.__config__.hongan4dKey==='radio'" :active-data="activeData" />
            <HONGAN4DCheckbox v-if="activeData.__config__.hongan4dKey==='checkbox'"
              :active-data="activeData" />
            <HONGAN4DSelect v-if="activeData.__config__.hongan4dKey==='select'" :active-data="activeData"
              :key="activeData.__config__.renderKey" :dictionaryOptions="dictionaryOptions"
              :dataInterfaceOptions="dataInterfaceOptions" />
            <HONGAN4DCascader v-if="activeData.__config__.hongan4dKey==='cascader'"
              :active-data="activeData" />
            <HONGAN4DTime v-if="activeData.__config__.hongan4dKey==='time'" :active-data="activeData" />
            <HONGAN4DTimeRange v-if="activeData.__config__.hongan4dKey==='timeRange'"
              :active-data="activeData" />
            <HONGAN4DDate v-if="activeData.__config__.hongan4dKey==='date'" :active-data="activeData" />
            <HONGAN4DDateRange v-if="activeData.__config__.hongan4dKey==='dateRange'"
              :active-data="activeData" />
            <HONGAN4DColorPicker v-if="activeData.__config__.hongan4dKey==='colorPicker'"
              :active-data="activeData" />
            <HONGAN4DRate v-if="activeData.__config__.hongan4dKey==='rate'" :active-data="activeData" />
            <HONGAN4DSwitch v-if="activeData.__config__.hongan4dKey==='switch'" :active-data="activeData" />
            <HONGAN4DSlider v-if="activeData.__config__.hongan4dKey==='slider'" :active-data="activeData" />
            <HONGAN4DDivider v-if="activeData.__config__.hongan4dKey==='divider'"
              :active-data="activeData" />
            <HONGAN4DUploadFz v-if="activeData.__config__.hongan4dKey==='uploadFz'"
              :active-data="activeData" />
            <HONGAN4DUploadImg v-if="activeData.__config__.hongan4dKey==='uploadImg'"
              :active-data="activeData" />
            <HONGAN4DComRight v-if="isCommon" :active-data="activeData" />
            <HONGAN4DAddress v-if="activeData.__config__.hongan4dKey==='address'"
              :active-data="activeData" />
            <TreeSelect v-if="activeData.__config__.hongan4dKey==='treeSelect'"
              :active-data="activeData" />
            <GroupTitle v-if="activeData.__config__.hongan4dKey==='groupTitle'"
              :active-data="activeData" />
            <RelationForm v-if="activeData.__config__.hongan4dKey==='relationForm'"
              :active-data="activeData" v-on="$listeners" :key="activeData.__config__.renderKey" />
            <RelationFormAttr v-if="activeData.__config__.hongan4dKey==='relationFormAttr'"
              :active-data="activeData" ref="relationFormAttr" />
            <HONGAN4DCalculate v-if="activeData.__config__.hongan4dKey==='calculate'"
              :active-data="activeData" ref="calculate" />
            <PopupSelect v-if="activeData.__config__.hongan4dKey==='popupSelect'"
              :active-data="activeData" />
            <PopupAttr v-if="activeData.__config__.hongan4dKey==='popupAttr'" :active-data="activeData"
              ref="popupAttr" />
            <Tab v-if="activeData.__config__.hongan4dKey==='tab'" :active-data="activeData" />
            <Collapse v-if="activeData.__config__.hongan4dKey==='collapse'" :active-data="activeData" />
            <template v-if="activeData.__config__.hongan4dKey==='barcode'">
            ...
            <template v-if="activeData.__config__.hongan4dKey==='qrcode'">
              <el-form-item label="控件标题">
                <el-input v-model="activeData.__config__.label" placeholder="请输入控件标题" />
              </el-form-item>
            ...
            <template v-if="activeData.__config__.hongan4dKey==='button'">
            ...
            <template v-if="isSystem">
              ...
              <el-form-item label="选择模板" v-if="activeData.__config__.hongan4dKey==='billRule'">
                <BillRule v-model="activeData.__config__.rule" placeholder="选择模板" />
              </el-form-item>
              <el-form-item label="显示内容" v-if="activeData.__config__.hongan4dKey==='currOrganize'">
            ...
            <template v-if="activeData.__config__.hongan4dKey==='card'">
            ...
            <!-- row 布局 -->
            <template v-if="activeData.__config__.hongan4dKey==='row'">
          ...
          <template v-if="activeData.__config__">
            <template v-if="!activeData.__config__.isSubTable">
              ...
              <el-form-item label="标题宽度"
                v-if="!['divider','HONGAN4DText','button','table','groupTitle ','card','row','tab','collapse'].includes(activeData.__config__.hongan4dKey)">
          ...
          <template v-if="modelType!=3 && modelType!=6 && webType!=3">
            ...
            <template v-if="webType==2">
              ...
              <el-form-item label="" v-if="formConf.hasPrintBtn">
                <HONGAN4D-TreeSelect :options="printTplList" v-model="formConf.printId"
                  placeholder="请选择打印模板" lastLevel clearable></HONGAN4D-TreeSelect>
              </el-form-item>
            </template>
          </template>
...
import HONGAN4DComInput from './RightComponents/ComInput'
import HONGAN4DTextarea from './RightComponents/Textarea'
import HONGAN4DText from './RightComponents/HONGAN4DText'
import HONGAN4DNumInput from './RightComponents/NumInput'
import HONGAN4DAmount from './RightComponents/HONGAN4DAmount'
import HONGAN4DPsdInput from './RightComponents/PsdInput'
import HONGAN4DRadio from './RightComponents/Radio'
import HONGAN4DCheckbox from './RightComponents/Checkbox'
import HONGAN4DSelect from './RightComponents/Select'
import HONGAN4DCascader from './RightComponents/Cascader/index'
import HONGAN4DTime from './RightComponents/Time'
import HONGAN4DTimeRange from './RightComponents/TimeRange'
import HONGAN4DDate from './RightComponents/Date'
import HONGAN4DDateRange from './RightComponents/DateRange'
import HONGAN4DColorPicker from './RightComponents/ColorPicker'
import HONGAN4DRate from './RightComponents/Rate'
import HONGAN4DSwitch from './RightComponents/Switch'
import HONGAN4DSlider from './RightComponents/Slider'
import HONGAN4DDivider from './RightComponents/Divider'
import HONGAN4DUploadFz from './RightComponents/UploadFz'
import HONGAN4DUploadImg from './RightComponents/UploadImg'
import HONGAN4DComRight from './RightComponents/ComRight'
import HONGAN4DAddress from './RightComponents/Address'
...
import HONGAN4DCalculate from './RightComponents/Calculate'
...
export default {
  ...
  computed: {
    ...
    isCommon() {
      return commonRightList.indexOf(this.activeData.__config__.hongan4dKey) > -1
    },
    isSystem() {
      return systemList.indexOf(this.activeData.__config__.hongan4dKey) > -1
    },
    drawingOptions() {
      let list = []
      const loop = (data, parent) => {
        if (!data) return
        if (data.__config__ && data.__config__.hongan4dKey !== 'table' && data.__config__.children && Array.isArray(data.__config__.children)) {
          loop(data.__config__.children, data)
        }
        if (Array.isArray(data)) data.forEach(d => loop(d, parent))
        if (data.__vModel__ && data.__config__.hongan4dKey !== 'table') {
          list.push(data)
        }
      }
  ...
  watch: {
    ...
    activeData(val) {
      if (val.__config__.hongan4dKey === 'relationFormAttr') {
        this.$nextTick(() => {
          this.$refs.relationFormAttr && this.$refs.relationFormAttr.getOptions()
        })
      }
      if (val.__config__.hongan4dKey === 'popupAttr') {
        this.$nextTick(() => {
          this.$refs.popupAttr && this.$refs.popupAttr.getOptions()
        })
      }
      if (val.__config__.hongan4dKey !== 'calculate') {
        this.$nextTick(() => {
          this.$refs.calculate && this.$refs.calculate.reloadExpressionTemp()
        })
      }
      if (!val.__config__.tableName && val.__config__.hongan4dKey !== 'table') {
  ...
  created() {
    ...
    if (!this.activeData.__config__.visibility) this.$set(this.activeData.__config__, "visibility", ["pc", "app"])
    if (!this.activeData.__config__.tableName && this.activeData.__config__.hongan4dKey !== 'table') {
  ...
  methods: {
    ...
    fieldChange1(val) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.hongan4dKey === 'table' && config.tableName === this.activeData.__config__.relationTable) {
          ...
          if (config && config.hongan4dKey != 'table' && config.children && Array.isArray(config.children)) {
    ...
    setDefaultOptions() {
      ...
      if (this.activeData.__vModel__ === undefined || this.activeData.__config__.hongan4dKey === 'table') return
      if (!this.activeData.__config__.tableName || this.activeData.__config__.tableName === this.mainTable) {
        ...
        if (!list.length) {
          ...
          let options = item.fields.map(o => ({
            ...o,
            realField: 'hongan4d_' + this.activeData.__config__.tableName + '_hongan4d_' + o.field,
          }))
    ...
    onTableNameChange(tableName) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          const e = list[i]
          const config = e.__config__
          if (config.hongan4dKey === 'table' && config.tableName === tableName) {
            boo = true
            break
          }
          if (config && config.hongan4dKey != 'table' && config.children && Array.isArray(config.children)) {
