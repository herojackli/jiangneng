...
const layouts = {
  colFormItem(h, element, index, parent) {
    ...
    if (config.hongan4dKey === 'HONGAN4DText') {
      return (
    ...
    if (config.hongan4dKey === 'groupTitle') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
    ...
    if (config.hongan4dKey === 'barcode') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <hongan4d-barcode format={element.format} lineColor={element.lineColor} background={element.background} width={element.width} height={element.height} staticText={element.staticText}></hongan4d-barcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.hongan4dKey === 'qrcode') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <hongan4d-qrcode size={element.size} colorLight={element.colorLight} colorDark={element.colorDark} staticText={element.staticText}></hongan4d-qrcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.hongan4dKey === 'button') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width="0">
            <hongan4d-button align={element.align} buttonText={element.buttonText} type={element.type} disabled={element.disabled}></hongan4d-button>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
  ...
  rowFormItem(h, element, index, parent) {
    ...
    const className = this.activeId === element.__config__.formId
      ...
      : 'drawing-row-item'
    if (element.__config__.hongan4dKey === 'tab') {
      return (
        <el-col span={24}>
    ...
    if (element.__config__.hongan4dKey === 'collapse') {
      return (
        <el-col span={24}>
          <el-row gutter={element.__config__.gutter} class={className}
            nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
    ...
    if (element.__config__.hongan4dKey === 'row') {
    ...
    if (element.__config__.hongan4dKey === 'card') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
    ...
    if (element.__config__.hongan4dKey === 'table') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
