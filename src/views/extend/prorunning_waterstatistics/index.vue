        ...
        <div class="HONGAN4D-common-layout-center">
            ...
            <div class="HONGAN4D-common-layout-main HONGAN4D-flex-main">
                ...
                <HONGAN4D-table v-loading="listLoading" :data="list" @sort-change='sortChange' >
                            <el-table-column prop="opentime" label="开机时间" width="0" align="left"
/>
                            <el-table-column prop="closetime" label="关机时间" width="0" align="left"
/>
                            <el-table-column prop="pumpid" label="机组名称" width="0" align="left"
/>
                            <el-table-column prop="diversionvolume" label="引水量" width="0" align="left"
/>
                            <el-table-column prop="duration" label="历时" width="0" align="left"
/>
    ...
    export default {
        ...
        data() {
            return {
                ...
                columnList: [
                    {prop: 'opentime', label: '开机时间'},
                    {prop: 'closetime', label: '关机时间'},
                    {prop: 'pumpid', label: '机组名称'},
                    {prop: 'diversionvolume', label: '引水量'},
                    {prop: 'duration', label: '历时'},
                ],
                                operationmodeOptions:[{"fullName":"引水","id":"引水"},{"fullName":"排涝","id":"排涝"}],
                            operationmodeProps:{"label":"fullName","value":"id"},
                            pumpidOptions:[],
                            pumpidProps:{"label":"PumpName","value":"Id"},
            }
        },
        computed: {
            menuId() {
        ...
        methods: {
            ...
            initData() {
                ...
                request({
                    url: `/api/example/ProRunning_WaterStatistics/getList`,
                    method: 'post',
                    data: _query
                }).then(res => {
                    var _list =[];
            ...
            handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/example/ProRunning_WaterStatistics/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
            ...
            download(data) {
                ...
                request({
                    url: `/api/example/ProRunning_WaterStatistics/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
