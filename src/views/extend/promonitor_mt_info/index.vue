                ...
                <el-form @submit.native.prevent>
                        ...
                        <el-col :span="6">
                                <el-form-item label="主变名称">
                                        <el-input v-model="query.mtname" placeholder="请输入" clearable>  </el-input>
                                </el-form-item>
                            </el-col>
                        <template v-if="showAll">
                                    <el-col :span="6">
                                        <el-form-item label="运行状态">
                                                <el-select v-model="query.runstatusenumid" placeholder="请选择"
                                                           clearable>
                                                    <el-option v-for="(item, index) in runstatusenumidOptions" :key="index"
...
/>
                            <el-table-column prop="mtname" label="主变名称" width="0" align="left"
/>
                            <el-table-column prop="runstatusenumid" label="运行状态" width="0" align="left"
/>
    ...
    export default {
        ...
        data() {
            return {
                ...
                columnList: [
                    ...
                    {prop: 'mtname', label: '主变名称'},
                    {prop: 'runstatusenumid', label: '运行状态'},
                    ...
                    {prop: 'orderindex', label: '排序'},
                            ...
                            runstatusenumidProps:{"label":"fullName","value":"id"},
            }
        },
        computed: {
            menuId() {
        ...
        methods: {
            ...
            initData() {
                ...
                request({
                    url: `/api/example/ProMonitor_MT_Info/getList`,
                    method: 'post',
                    data: _query
                }).then(res => {
                    var _list =[];
            ...
            handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/example/ProMonitor_MT_Info/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
            ...
            download(data) {
                ...
                request({
                    url: `/api/example/ProMonitor_MT_Info/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
