                        ...
                        <template v-if="showAll">
                                    <el-col :span="6">
                                        <el-form-item label="测站类型">
                                                <el-select v-model="query.stationTypeenumid" placeholder="请选择"
                                                           clearable>
                                                    <el-option v-for="(item, index) in stationTypeenumidOptions" :key="index"
...
/>
                            <el-table-column prop="stnm" label="测站名称" width="0" align="left"
/>
                            <el-table-column prop="stcd" label="测站编码" width="0" align="left"
/>
                                <el-table-column label="测站类型" width="0" prop="stationTypeenumid" algin="left"
>
                                    <template slot-scope="scope">
                                        {{ scope.row.stationTypeenumid | dynamicText(stationTypeenumidOptions) }}
                                    </template>
                                </el-table-column>
                            <el-table-column prop="adcd" label="行政区划" width="0" align="left"
/>
    ...
    export default {
        ...
        data() {
            return {
                ...
                columnList: [
                    ...
                    {prop: 'stationTypeenumid', label: '测站类型'},
                    {prop: 'adcd', label: '行政区划'},
                    ...
                    {prop: 'orderindex', label: '排序'},
                            ...
                            stationTypeenumidProps:{"label":"fullName","value":"id"},
            }
        },
        computed: {
            menuId() {
        ...
        methods: {
            ...
            initData() {
                ...
                request({
                    url: `/api/example/ProMonitor_WaterInfo/getList`,
                    method: 'post',
                    data: _query
                }).then(res => {
                    var _list =[];
            ...
            handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/example/ProMonitor_WaterInfo/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
            ...
            download(data) {
                ...
                request({
                    url: `/api/example/ProMonitor_WaterInfo/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
