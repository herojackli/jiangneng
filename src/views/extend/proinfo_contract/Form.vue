                ...
                <template v-if="!loading">
                    ...
                    <el-col :span="24">
                        <el-form-item label="封面图片" prop="coverprofileid">
                            <HONGAN4D-UploadImg v-model="dataForm.coverprofileid" :fileSize="5" sizeUnit="MB"
                                :limit="9">
...
export default {
    ...
    methods: {
        ...
        init(id, isDetail) {
            ...
            this.$nextTick(() => {
                ...
                if (this.dataForm.id) {
                    this.loading = true
                    request({
                        url: '/api/example/ProInfo_Contract/' + this.dataForm.id,
                        method: 'get'
                    }).then(res => {
        ...
        request() {
            ...
            if (!this.dataForm.id) {
                request({
                    url: '/api/example/ProInfo_Contract',
                    method: 'post',
                    data: _data
                }).then((res) => {
                    this.$message({
