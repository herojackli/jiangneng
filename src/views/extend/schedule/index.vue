...
export default {
  ...
  methods: {
    datesRender(calendar) {
      ...
      this.startTime = this.hongan4d.toDate(view.activeStart, "yyyy-MM-dd HH:mm")
      this.endTime = this.hongan4d.toDate(view.activeEnd, "yyyy-MM-dd HH:mm")
      this.initData()
    },
    handleDateClick(arg) {
      let startTime = this.hongan4d.toDate(arg.date, "yyyy-MM-dd HH:00"),
        clickTime = this.hongan4d.toDate(arg.date, "yyyyMMdd"),
        currTime = this.hongan4d.toDate(new Date(), "yyyyMMdd");
      if (clickTime < currTime) return
      if (clickTime == currTime) {
        let thisDate = new Date();
        thisDate.setHours(thisDate.getHours() + 1);
        startTime = this.hongan4d.toDate(thisDate, "yyyy-MM-dd HH:00");
      }
      this.addOrUpdateHandle('', startTime)
      this.addOrUpdateHandle('', new Date(startTime).getTime())
    },
    eventClick(data) {
      if (this.hongan4d.toDate(data.event.start, "yyyyMMddHHmm") > this.hongan4d.toDate(new Date(), "yyyyMMddHHmm")) {
