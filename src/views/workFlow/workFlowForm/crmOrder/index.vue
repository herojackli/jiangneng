...
export default {
  ...
  methods: {
    ...
    count(row) {
      //金额 = 数量*单价
      row.amount = this.hongan4d.toDecimal(parseFloat(row.price) * parseFloat(row.qty))
      //折扣价 = (单价*折扣)
      var discountPrice = row.price * (row.discount ? row.discount / 100 : 1);
      //实际单价 = 折扣价 * (1 + (税率 / 100))
      row.actualPrice = this.hongan4d.toDecimal(discountPrice * (1 + (row.cess ? row.cess / 100 : 0)));
      //实际金额
      row.actualAmount = this.hongan4d.toDecimal(parseFloat(row.actualPrice) * parseFloat(row.qty))
    }
  }
}
</script>
